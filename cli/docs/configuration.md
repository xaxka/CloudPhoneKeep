...
| `CPK_STREAM_SCALE` | `100` | 采集分辨率百分比（30-100；控制台「设置→分辨率」可运行时调整）：<100 时 Chrome 编码前先缩小，编码 CPU 与带宽按像素数近线性下降（如 75 ≈ 省 44%），触摸坐标不受影响。弱机推荐 75-90。原理（采集参数而非二次处理，见 [deploy.md](deploy.md)「CPK_STREAM_SCALE 的原理」） |
