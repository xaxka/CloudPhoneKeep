...
// 控制页所在端口（mini 引擎端口）
const ENG_PORT = 8933;
const A_PORT = 9444; // Chrome A CDP
const B_PORT = 9555; // Chrome B CDP
const BASE_FLAGS = [
...
async function startCloudChrome() {
  procA = spawn(SHELL, [...BASE_FLAGS, `--remote-debugging-port=${A_PORT}`, '--user-data-dir=/tmp/cpk-rA', 'about:blank'], { stdio: 'ignore' });
...
async function startControlChrome() {
  procB = spawn(SHELL, [...BASE_FLAGS, `--remote-debugging-port=${B_PORT}`, '--user-data-dir=/tmp/cpk-rB', `http://127.0.0.1:${ENG_PORT}/`], { stdio: 'ignore' });
