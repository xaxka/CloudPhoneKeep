...
**帧率与 CPU 正相关，引擎做了三重省 CPU 优化**：① Chrome 端按目标帧率
降低 JPEG 编码量（everyNthFrame：10fps 时每 6 个合成器帧才编码 1 帧，
编码 CPU 约降为 1/6）；② 限帧窗口内被丢弃的帧在解码前直接跳过（不再
为注定丢弃的帧做 base64 解码）；③ base64 解码改为 O(1) 查表（旧实现
逐字符扫 64 项字母表）。画质随帧率联动：低帧率补画质（q55）、高帧率
降画质（q35）以兼顾带宽。
