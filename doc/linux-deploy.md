...
桌面端右侧是操作栏；**手机等窄屏**为底部快捷坞 + 抽屉两级交互：
- **底部快捷坞**（常驻）：`首页`/`刷新`/`粘贴`/`全屏` 四个高频操作一步直达，
  中央 iOS 风格白色圆点呼出抽屉控制台；
- **底部抽屉**（完整控制台）：点圆点或遮罩弹出，再点圆点/点遮罩/**按住顶部把手下滑**收起
  （把手是手势唯一起点，抽屉内容不会误触关闭）。抽屉打开期间快捷坞自动隐藏。
...
**控制台「设置 → 帧率」可运行时限制帧率**（1-60，默认 25；低配 ARM64 盒子
建议 8-12，初始值可用环境变量 `CPK_FPS` 设定）。
...
2. **everyNthFrame = floor(60/fps)**：Chrome 端编码节流的第二保险
   （部分 build 无视此参数）。
3. **丢帧先于解码 + base64 O(1) 查表**：Rust 侧只为真正推送的帧解码。
4. **可调旋钮**：`CPK_JPEG_QUALITY`（默认 50，降质量省编码 CPU/带宽）、
   `CPK_STREAM_SCALE`（默认 100；如 75 = 分辨率缩 75%，编码量按像素
   近线性下降，触摸坐标是 CSS 坐标系不受影响）。
观看中每 30s 日志输出一行帧流统计（Chrome 出帧 N/s → 解码推送 M/s），
可据此判断节流是否生效（出帧率 ≈ 目标帧率即生效）。
