...
- 实时画面流（`/stream.mjpg`）判读：连接应为亚秒级（订阅即时应答 + 首帧
  截图兜底）；**静态页面徽标 0-1 fps 属正常**——合成器无更新即无新帧，
  服务端每 2s 心跳重发上一帧保连接，不是卡顿。页面有动画/视频时帧率应显著
  上去（帧信箱丢旧保新 + screencast ack 已修复，本地实测动画页 60fps；
  弱机/弱网下由 CPU 编码与带宽决定，通常 5-25fps）。若动画页仍长期 1-2fps：
  查引擎是否陷在长采样（日志「慢调用诊断」）。频繁闪「连接实时画面…」=
  流被反复重建：查日志里的 `WS:` 传输错误（CDP 会话在重建）、
  `生产侧心跳丢失`（正常重连，随会话重建）或 `首帧 30s 未至`（引擎极端繁忙）
