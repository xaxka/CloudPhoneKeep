...
- 「窗口设置」打开**窗口设置**小窗（仅分辨率输入 + 保存，只改当前窗口、不写配置——与原版 settingWin 一致；单实例，帐号启动时即预创建隐藏，点菜单瞬时弹出；关闭=隐藏，可反复秒开）
- `Ctrl+N`（N=老板键索引）瞬间隐藏/呼出，`Ctrl+U` 呼出地址栏（回车跳转、Esc 关闭，与原版一致）
- 每个云手机窗口有**自己的托盘图标**（与原版每个 webForm 各建一个托盘一致）：**左键单击打开窗口，右键弹出菜单**（显示(●)/隐藏(●)/退出，另附加 新开账号 / 打开数据目录）
