...
| `enableDefaultContextMenus(false)`、触点光标注入、`onDocumentInit` 重装 | 右键屏蔽 + 本地内嵌**圆点触控光标**（复刻原版 Dotter 风格，原外链已失效；默认开）+ 每次导航自动重装 |
| 桌面浏览器打开 H5 即可用鼠标操控（页面自带鼠标→触摸模拟器） | **鼠标→触摸操控模拟**：WebView2 里页面的模拟器不加载（`ontouchstart` 存在），由注入脚本移植同款模拟器补上，鼠标拖动=滑动、不再选择文本 |
...
| 看日志 | `logs/cpk-YYYYMMDD-p<进程号>.log`（程序一启动就会写入，含版本、pid、exe 目录、回环端口、WebView2 状态；任何启动失败也会落盘） |
