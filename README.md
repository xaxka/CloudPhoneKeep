...
- **不在任务栏显示**：设置窗口、云手机窗口均跳过 Windows 任务栏（`skipTaskbar` / `skip_task_bar`），交互通过各云手机窗口的独立托盘完成。**不再有启动即常驻的全局托盘**——「新开账号」设置窗口只是一个界面，启动阶段托盘区保持干净；设置窗口隐藏后可由任一云手机窗口托盘菜单的「新开账号」项唤回（进入云手机窗口时一定伴随其独立托盘）
- 「首页」「窗口置顶」移至托盘右键菜单
