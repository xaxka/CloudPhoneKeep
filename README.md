...
- **不在任务栏显示**：设置窗口、云手机窗口均跳过 Windows 任务栏（`skipTaskbar` / `skip_task_bar`），全部交互通过托盘完成。除每槽位独立托盘外，启动即创建一个**全局托盘**（程序生命周期内始终存在），左键单击 = 显示设置窗口，右键菜单 = 新开账号 / 退出——保证设置窗口被隐藏后任何时候都能通过它再次唤回
- 「首页」「窗口置顶」移至托盘右键菜单
