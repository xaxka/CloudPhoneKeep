...
| `web.aardio` 菜单五项：云手机首页/旋转/窗口置顶/设置/检查更新 | 同五项原生菜单；「窗口设置」= 窗口设置小窗（单实例，建窗期间防连击去重） |
...
| `enableDefaultContextMenus(false)`、触点光标注入、`onDocumentInit` 重装 | 右键屏蔽 + 本地内嵌**安卓官方风格触点光标**（Android 绿圆环+白色触点面；固定启用，无开关）+ 每次导航自动重装 |
...
| 看日志 | `logs/cpk-YYYYMMDD.log`（每天一个文件，多实例同写一份、行内 `[pid=N]` 区分；程序一启动就会写入，含版本、pid、exe 目录、回环端口、WebView2 状态；任何启动失败也会落盘） |
...
- **安卓官方风格触点光标**（本地内嵌资源，固定启用，零外部依赖）、**屏蔽页面右键菜单**
