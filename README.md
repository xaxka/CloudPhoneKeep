...
- 「设置」打开**窗口设置**小窗（仅分辨率输入 + 保存，只改当前窗口、不写配置——与原版 settingWin 一致）
- `Ctrl+N` 老板键瞬间隐藏/呼出，`Ctrl+U` 呼出地址栏（回车跳转、Esc 关闭，与原版一致）
- 每个云手机窗口有**自己的托盘图标**（与原版每个 webForm 各建一个托盘一致）：右键菜单 显示(●)/隐藏(●)/退出，另附加 打开设置（新开帐号）/ 打开日志目录

## 退出语义（与原版完全一致）

- **云手机窗口右上角 X = 退出整个程序**（原版 `webForm.onClose → win.quitMessage()`）
- **设置窗口右上角 X = 退出整个程序**（原版 `loginForm.onClose → win.quitMessage()`）
- **托盘右键 → 退出 = 退出整个程序**（原版 `tray.delete + close + quitMessage`）

要隐藏窗口继续保活，用老板键 `Ctrl+N` 或托盘「隐藏」，不要点 X。
...
| `showWebForm` 成功 → `loginForm.show(false)` | 启动成功后隐藏设置窗口 |
| `login/web.aardio` 关闭窗口 → `win.quitMessage()` | 任一窗口点 X → 销毁全部窗口并退出进程 |
| `web.aardio` 菜单五项：云手机首页/旋转/窗口置顶/设置/检查更新 | 同五项原生菜单；「设置」= 窗口设置小窗（单实例，可重复打开） |
| 「设置」settingWin：仅分辨率 + 保存，内存生效不落盘 | `winset` 小窗：改窗口与会话内覆盖，重启恢复配置值 |
| 「旋转」交换 userInfo 宽高 + `go(location)` 刷新，不落盘 | 会话内交换宽高 + `location.reload()`，不落盘 |
...
| CDP `Network.setCookies` domain=`.139.com` 导航前生效 | document-start 按平台域(`.139.com`/`.wo-adv.cn`)一次性写入 + 首次自动刷新，使首个请求即携带；不周期重刷 |
| Cookie 解析 `string.lines(str, ";\s*")` | 同时兼容 `a=b; c=d` 单行与 `name=value` 多行 |
| `enableDefaultContextMenus(false)`、触点光标注入、`onDocumentInit` 重装 | 右键屏蔽 + 本地内嵌光标（替代远程 .cur，消除外链）+ 每次导航自动重装 |
...
| 现象 | 原因与处理 |
| :--- | :--- |
| 点「进入」弹红色横幅 | 横幅里就是具体原因。常见：老板键 Ctrl+N 被其他程序占用（此时窗口仍会正常打开，仅无老板键）；数据目录被另一个实例占用（勿同时运行两份程序） |
| 提示缺少 WebView2 | 程序会自动打开微软官方下载页，安装「Evergreen 独立安装包」后重开即可 |
| 页面空白/加载不出来 | 15 秒后页面底部会出现黄色重试条（重新加载 / 回云手机首页），同时 `logs/` 里有对应的 `[error]` 记录 |
| 看日志 | `logs/cpk-YYYYMMDD.log`（程序一启动就会写入，含版本、exe 目录、回环端口、WebView2 状态；任何启动失败也会落盘） |
...
3. 在打开的窗口中完成云手机登录；之后用 `Ctrl+N` 或托盘「隐藏」收起窗口，保活继续
