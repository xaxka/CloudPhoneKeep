...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // 不再创建全局托盘：「新开账号」设置窗口只是一个界面，启动阶段托盘区应保持干净，
            // 不该一启动就常驻一个图标（此前全局托盘启动即出现且退出后残留不消失）。
            // 设置窗口隐藏后仍可唤回：进入任一云手机窗口时一定伴随其独立托盘，
            // 该托盘菜单的「新开账号」项即可显示设置窗口（见 browser.rs slot_tray_menu）。
            // 全局托盘原本的「未开任何云手机窗口时也能唤回」场景在此不成立——
            // 启动阶段设置窗口要么可见要么程序已退出（点 X = 退出），不存在「隐藏了唤不回」。
            // 槽位托盘按原版语义：每个云手机窗口启动时各建自己的托盘（见 browser.rs create_slot_tray）
