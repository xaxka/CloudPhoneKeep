...
fn main() {
    ...
    tauri::Builder::default()
        .manage(AppState::new())
        // 【多 exe 多开】不注册单实例插件：每运行一个 exe 即一个独立实例，
        // 各自拥有自己的设置窗口、云手机窗口与独立托盘图标（再开一个云手机 =
        // 再运行一个 exe）。单实例锁会把第二次启动直接聚焦旧实例并退出，
        // 多开根本起不来。
        ...
        .setup(move |app| {
            ...
            // 不创建全局托盘：多开走多 exe 运行（再开一个云手机 = 再运行一个
            // 程序实例），托盘菜单也没有「新开账号」入口，启动阶段托盘区保持干净。
            // 设置窗口只在启动时显示，点「进入」后隐藏且本实例不会再唤回；
            // 槽位托盘按原版语义：每个云手机窗口启动时各建自己的托盘
