...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // 全局托盘：始终存在（程序生命周期内有效），左键显示设置窗口，右键菜单：新开账号 / 退出。
            // 配合「不在任务栏显示」语义：设置窗口、云手机窗口都跳过任务栏，用户隐藏后
            // 始终能通过全局托盘再次唤回——即便此时还没有任何云手机窗口（即没有任何槽位托盘）
            browser::create_global_tray(app.handle());
            // 槽位托盘不再全局创建：还原原版语义——每个云手机窗口启动时各建自己的托盘
